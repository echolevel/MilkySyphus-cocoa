...
PPButton::PPButton(pp_int32 id, PPScreen* parentScreen, EventListenerInterface* eventListener, 
	...
	update(update),
	verticalText(false),
    showsImage(false), // Bren July 2015
	flat(true), // Bren July 2015
	autoSizeFont(true),
	offset(0,0),
	invertShading(false),
...
void PPButton::paint(PPGraphicsAbstract* g)
{
    ...
    PPColor bColor2 = *color; // Bren Nov 2014 start
	...
	{			
        ...
        // Bren July 2015
        // Use Processing sketch brenImg2Hex to turn Gimp <imagefile>.data into LogoCustom.cpp array
        if(showsImage) {
            
            PPPoint location = this->location;
            //g->setRect(location.x, location.y, location.x + size.width, location.y + size.height);
            g->setColor(*color);
            g->blit(LogoCustom::rawData, location, PPSize(LogoCustom::width, LogoCustom::height), LogoCustom::width*4, 4);
        }
		...
		if (text)
		{
            ...
            if (!verticalText)
            {
                if(!showsImage) {
                    ...
                    pp_int32 cx = (size.width>>1)-(font->getStrWidth(text)>>1) + location.x;
                    pp_int32 cy = (size.height>>1)-(font->getCharHeight()>>1) + location.y + 1;
                    g->drawString(text, cx+offset.x + 4, cy *1.8 - (font->getCharHeight()>>1) +offset.y);
                }
            }
            else
            {
