...
void Tracker::initListboxesSection(pp_int32 x, pp_int32 y)
{
    ...
    // Bren Nov 2014 - I've just gone pure nuts adding values to all this shit to bump it along to the right - fit some gfx in? 'Samples' doesn't need to be anywhere near as wide. Warning: this might fuck up at low res. I don't care. Alternative: get Samples shrunk to half the height (doesn't need to be that tall either) and whack in the FTII logo!
    
    
    // Instruments
#ifndef __LOWRES__
    button = new PPButton(BUTTON_INSTRUMENT, screen, this, PPPoint(x+4 + size, y+dy-2), PPSize(screen->getWidth() < 800 ? 3*8+4 : 11*8+4, 12), false, true, false);
...
#else
    button = new PPButton(BUTTON_INSTRUMENT, screen, this, PPPoint(x+4, y+dy-2), PPSize(11*8+4+size, 11), false, true, false);
    button->setText("Instruments");
    button->setColor(TrackerConfig::colorThemeMain);
    button->setTextColor(PPUIConfig::getInstance()->getColor(PPUIConfig::ColorStaticText));
    button->setPressed(false);
    {
        PPStaticText* staticText = new PPStaticText(STATICTEXT_INSTRUMENTS_ALTERNATIVEHEADER, screen, this, PPPoint(x+5, y+dy), "Samples / Ins:", true);
        staticText->hide(true);
        container->addControl(staticText);
        
        staticText = new PPStaticText(STATICTEXT_INSTRUMENTS_ALTERNATIVEHEADER2, screen, this, PPPoint(x+4 + 14*8, y+dy), "xx", false);
        staticText->hide(true);
        container->addControl(staticText);
    }
#endif
    ...
    button = new PPButton(BUTTON_INSTRUMENTS_PLUS, screen, this, PPPoint(x+button->getSize().width+8+size, y+dy+tinyButtonOffset), PPSize(15, tinyButtonHeight));
    button->setText(TrackerConfig::stringButtonPlus);
    container->addControl(button);
...
#ifndef __LOWRES__
    button = new PPButton(BUTTON_INSTRUMENTEDITOR_CLEAR, screen, sectionInstruments, PPPoint(x+5 + size*2 - 2 - 92, y+dy+tinyButtonOffset), PPSize(30, tinyButtonHeight));
    button->setFont(PPFont::getFont(PPFont::FONT_TINY));
    button->setText("Zap");
    container->addControl(button);
    
    button = new PPButton(BUTTON_INSTRUMENTEDITOR_LOAD, screen, sectionInstruments, PPPoint(x+5 + size*2 - 2 - 61, y+dy+tinyButtonOffset), PPSize(30, tinyButtonHeight));
    button->setFont(PPFont::getFont(PPFont::FONT_TINY));
    button->setText("Load");
    container->addControl(button);
    
    button = new PPButton(BUTTON_INSTRUMENTEDITOR_SAVE, screen, sectionInstruments, PPPoint(x+5 + size*2 - 2 - 30, y+dy+tinyButtonOffset), PPSize(30, tinyButtonHeight));
    button->setFont(PPFont::getFont(PPFont::FONT_TINY));
    button->setText("Save");
    container->addControl(button);
#else
    button = new PPButton(BUTTON_INSTRUMENTS_FLIP, screen, this, PPPoint(container->getLocation().x + size + container->getSize().width - 27, y+dy+tinyButtonOffset - 1), PPSize(24, 11), false);
    button->setText("Flip");
    button->setColor(TrackerConfig::colorThemeMain);
    button->setTextColor(PPUIConfig::getInstance()->getColor(PPUIConfig::ColorStaticText));
    container->addControl(button);
    ...
    //listBoxInstruments = new PPListBox(LISTBOX_INSTRUMENTS, screen, this, PPPoint(x+4 + size, y + 7 + dy + dy), PPSize(size+myDx,height-(10+2*dy)), true, true, true, true);
    listBoxInstruments = new PPListBox(LISTBOX_INSTRUMENTS, screen, this, PPPoint(x+4 + size, y + 7 + dy + dy), PPSize(size+myDx,height-(60+2*dy)), true, true, true, true); // Bren July 2015
    ...
    // Bren July 2015 (false, false, false is: border, clickable, update)
    //button = new PPButton(BUTTON_LOGO, screen, this, PPPoint(x+2, y+dy-2), PPSize(screen->getWidth() < 800 ? 157 : 157, 113), false, false, false);
    button = new PPButton(BUTTON_LOGO, screen, this, PPPoint(x+2, y+dy-2), PPSize(screen->getWidth() < 800 ? 1 : 1, 1), false, false, false); // image overflows, but button doesn't block clicks for other buttons
    button->setColor(TrackerConfig::colorThemeMain);
    button->setImage(true);
    container->addControl(button);
    
    // Custom text (replaces 'Samples')
    PPStaticText* customText = new PPStaticText(STATICTEXT_CUSTOMTEXT, NULL, NULL, PPPoint(x+myDx+12, y + height/2 - 2 +dy), "-=MIGGY TRACKER=-", true);
    PPStaticText* customText2 = new PPStaticText(STATICTEXT_CUSTOMTEXT2, NULL, NULL, PPPoint(x+myDx+110, y + height/2 + 42 + dy), "Sys/UP", true);
    customText->setColor(PPUIConfig::getInstance()->getColor(PPUIConfig::ColorDefaultButtonText));
    customText2->setColor(PPUIConfig::getInstance()->getColor(PPUIConfig::ColorDefaultButtonText));
    //container->addControl(customText);
    container->addControl(customText2);
...
#ifndef __LOWRES__
    button = new PPButton(BUTTON_SAMPLE_EDIT_CLEAR, screen, sectionSamples, PPPoint(x+2 + size - 2 - 92 +159, y + height/2 - 6 +dy+tinyButtonOffset + 11), PPSize(30, tinyButtonHeight));
    button->setFont(PPFont::getFont(PPFont::FONT_TINY));
    button->setText("Clear");
    container->addControl(button);	 // Bren July 2015 - HIDE SAMPLES!!!
    
    button = new PPButton(BUTTON_SAMPLE_LOAD, screen, sectionSamples, PPPoint(x+2 + size - 2 - 61 +159, y + height/2 - 6 +dy+tinyButtonOffset + 11), PPSize(30, tinyButtonHeight));
    button->setFont(PPFont::getFont(PPFont::FONT_TINY));
    button->setText("Load");
    container->addControl(button);  // Bren July 2015 - HIDE SAMPLES!!!
    
    button = new PPButton(BUTTON_SAMPLE_SAVE, screen, sectionSamples, PPPoint(x+2 + size - 2 - 30 +159, y + height/2 - 6 +dy+tinyButtonOffset + 11), PPSize(30, tinyButtonHeight));
    button->setFont(PPFont::getFont(PPFont::FONT_TINY));
    button->setText("Save");
    container->addControl(button);  // Bren July 2015 - HIDE SAMPLES!!!
#endif
    
    PPStaticText* staticText = new PPStaticText(STATICTEXT_SAMPLEHEADER, NULL, NULL, PPPoint(x+myDx+162, y + height/2 + 5 +dy), "Samples", true);
    
    
    container->addControl(staticText);  // Bren July 2015 - HIDE SAMPLES!!!
    
    //listBoxSamples = new PPListBox(LISTBOX_SAMPLES, screen, this, PPPoint(x+2 , y + height/2 + 3 + dy + dy), PPSize(size,height-(10+16*dy)), true, true, true, true);
    listBoxSamples = new PPListBox(LISTBOX_SAMPLES, screen, this, PPPoint(x+64 +96 , y + height/2 + 12 + dy + dy), PPSize(size,height-(20+16*dy)+2), true, true, true, true);
    ...
    container->addControl(listBoxSamples); // Bren July 2015 - HIDE SAMPLES!!!
