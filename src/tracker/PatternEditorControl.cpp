...
lastAction(RMouseDownActionInvalid), RMouseDownInChannelHeading(-1)
{
    ...
    //hBottomScrollbar = new PPScrollbar(3, parentScreen, this, PPPoint(location.x + SCROLLBARWIDTH, location.y + size.height - SCROLLBARWIDTH), size.width - SCROLLBARWIDTH*2, true); // Bren Nov 2014
    hBottomScrollbar = new PPScrollbar(3, parentScreen, this, PPPoint(location.x, location.y + size.height - SCROLLBARWIDTH), size.width, true);
...
void PatternEditorControl::paint(PPGraphicsAbstract* g)
{
    ...
    //g->setRect(location.x+SCROLLBARWIDTH, location.y+SCROLLBARWIDTH, location.x + size.width - SCROLLBARWIDTH, location.y + size.height - SCROLLBARWIDTH); // Bren Nov 2014
    ...
    for (pp_int32 i2 = startIndex;; i2++)
    {
        ...
        //pp_int32 px = location.x + SCROLLBARWIDTH; // Bren Nov 2014
        pp_int32 px = location.x;
        
        //pp_int32 py = location.y + (i-startIndex) * font->getCharHeight() + SCROLLBARWIDTH + (font->getCharHeight() + 4); Bren Nov 2014
        ...
        for (j = startPos; j < numVisibleChannels; j++)
        {
            ...
            pp_int32 py = location.y + SCROLLBARWIDTH/3;
            ...
            
            {
                ...
                //PPRect rect(px, py, px+slotSize, py + font->getCharHeight()+1); // Bren Nov 2014
            ...
            else
            {
                if (!(j&1))
                    //g->setColor(hiLightPrimary);
                    g->setColor(textColor); // Bren Nov 2014
                else
                    g->setColor(textColor);
            ...
            //if (muteChannels[j]) strcat(name, " <Mute>"); // Bren Nov 2014 - don't need this
            
            //g->drawString(name, px + (slotSize>>1)-(((pp_int32)strlen(name)*font->getCharWidth())>>1), py+1);
            g->drawString(name, px - 8, py+1); // Bren Nov 2014 - left-align channel numbers
        }
        ...
        for (j = startPos; j < numVisibleChannels; j++)
        {
                ...
                i == cursor.row)
            {
                ...
                for (pp_int32 k = cursorPositions[cursor.inner]+1; k < cursorPositions[cursor.inner]+cursorSizes[cursor.inner]-1; k++)
                    g->drawVLine(py, py + font->getCharHeight(), px + k); // Bren Nov 2014 - FT2-style bevelled cursor
                ...
                c2.scaleFixed(87163);
                g->setColor(c2);
                //g->drawHLine(px + cursorPositions[cursor.inner], px + cursorPositions[cursor.inner]+cursorSizes[cursor.inner], py - 1); // Bren Nov 2014
                g->setColor(c);
                //g->drawHLine(px + cursorPositions[cursor.inner], px + cursorPositions[cursor.inner]+cursorSizes[cursor.inner], py + font->getCharHeight()); // Bren Nov 2014
            }
    ...
    for (j = startPos; j < numVisibleChannels; j++)
    {
        ...
        //pp_int32 px = (location.x + (j-startPos) * slotSize + SCROLLBARWIDTH) + (getRowCountWidth() +4); // Bren Nov 2014
    ...
    // draw scrollbars
    //hTopScrollbar->paint(g); // Bren Nov 2014 - hiding top, left and right scrollbars (pointless waste of space)
...
{
    //visibleWidth = size.width - (getRowCountWidth() + 4) - SCROLLBARWIDTH*2; // Bren Nov 2014
    visibleWidth = size.width - (getRowCountWidth() + 4);
    //visibleHeight = size.height - (font->getCharHeight() + 4) - SCROLLBARWIDTH*2; // Bren Nov 2014
